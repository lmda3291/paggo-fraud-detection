...
export async function GET() {
  const stats = db.prepare(`
    ...
    FROM transactions
  `).get() as { confirmedSuspicious: number; falsePositives: number; pendingReview: number; totalFlagged: number };

  const reviewed = stats.confirmedSuspicious + stats.falsePositives;
  const reviewRate = stats.totalFlagged > 0
    ? Math.round((reviewed / stats.totalFlagged) * 1000) / 10
    : 0;
